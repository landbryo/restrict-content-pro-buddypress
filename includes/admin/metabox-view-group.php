...
$sub_levels        = groups_get_groupmeta( $group_id, 'rcp_subscription_level', true );
$set_level         = is_array( $sub_levels ) ? '' : $sub_levels;
$access_level      = groups_get_groupmeta( $group_id, 'rcp_access_level', true );
$access_level      = is_numeric( $access_level ) ? absint( $access_level ) : '';
$show_excerpt      = groups_get_groupmeta( $group_id, 'rcp_show_excerpt', true );
$hide_in_feed      = groups_get_groupmeta( $group_id, 'rcp_hide_from_feed', true );
$user_role         = groups_get_groupmeta( $group_id, 'rcp_user_level', true );
...
<div id="rcp-metabox-field-restrict-by" class="rcp-metabox-field">
	...
	<p>
		<?php _e( 'Select who should have access to view this group. Note if the user is a member of the group, these restrictions will be ignored.', 'rcpbp' ); ?>
